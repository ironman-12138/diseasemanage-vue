...
<script>
import {getLoginLogList,deleteById} from '@/api/loginlog'
export default {
  data() {
    return {
      sels: [], //选中的值显示
  ...
  methods: {
    ...
    async getLoginLogList() {
      getLoginLogList(this.queryMap).then(res => {
          if (!res.success) {
            return this.$message.error("获取列表失败:"+res.data.errorMsg);
          } else {
            this.total = res.data.log.total;
            this.LoginLogData = res.data.log.dataList;
          }
      })
    },
    //删除登入日志
